...
def main(show_plot=True):
    ...
    strategy_name = "moving-average-oscillators"
    ...
    buy_conditions   = (ohlc["ema10"] > ohlc["ema40"]) & (ohlc["Open"] > ohlc["ema40"]) & (ohlc["Open"] < ohlc["ema10"]) & \
                       (ohlc["Close"] > ohlc["ema10"]) & (ohlc["rsi"] < 50)               


    sell_conditions  = (ohlc["ema10"] < ohlc["ema40"]) & (ohlc["Open"] < ohlc["ema40"]) & (ohlc["Open"] > ohlc["ema10"]) & \
                       (ohlc["Close"] < ohlc["ema10"]) & (ohlc["rsi"] > 50)
