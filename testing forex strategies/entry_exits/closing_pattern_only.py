...
def main(show_plot=True):
    ...
    strategy_name = "closing-pattern-only"
    # Read in the data
    ohlc  = read_data()
    ...
    buy_conditions   = (ohlc["Close_1"] > ohlc["Close_3"]) & (ohlc["Close"] > ohlc["Close_2"]) & (ohlc["Close_2"] > ohlc["Close_1"])
    sell_conditions  = (ohlc["Close_1"] < ohlc["Close_3"]) & (ohlc["Close"] < ohlc["Close_2"]) & (ohlc["Close_2"] < ohlc["Close_1"])
