...
def candlestick_pivot_point_1():
    ...
    patterns  = ohlc_merged.ta.cdl_pattern(name=["matchinglow"])

    ohlc_merged["matchinglow"] = patterns["CDL_MATCHINGLOW"] 

    ohlc_merged.loc[:,"bullish_close"] = np.where(ohlc_merged["matchinglow"]==100,ohlc_merged["Close"],np.nan) 
    ohlc_merged.loc[:,"bearish_close"] = np.where(ohlc_merged["matchinglow"]==-100,ohlc_merged["Close"],np.nan)

    # Signal conditions
    buy_conditions  = (ohlc_merged["bullish_close"] >  ohlc_merged["R2"]) & (ohlc_merged["matchinglow"]==100)
    sell_conditions = (ohlc_merged["bearish_close"] <  ohlc_merged["S2"]) & (ohlc_merged["matchinglow"]==-100)
...
def candlestick_pivot_point_2():
    ...
    # Signal conditions
    buy_conditions  = (ohlc_merged["bullish_close"] <  ohlc_merged["S2"]) & (ohlc_merged["matchinglow"]==100)
    sell_conditions = (ohlc_merged["bearish_close"] >  ohlc_merged["R2"]) & (ohlc_merged["matchinglow"]==-100)
...
def main():
    ...
    # Calculate the patterns
    patterns  = ohlc.ta.cdl_pattern(name=["matchinglow"])

    
    # Signal position points (i.e. for plotting)
    buy_conditions  = (patterns["CDL_MATCHINGLOW"]==100)
    sell_conditions = (patterns["CDL_MATCHINGLOW"]==-100)
