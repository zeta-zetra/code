...
def main(show_plot=True):
    ...
    strategy_name = "filtered-entry-exponentially-better"
    ...
    buy_conditions   = (ohlc["ema10"] > ohlc["ema40"]) & (ohlc["lag_ema10"] < ohlc["lag_ema40"]) & (ohlc["range_1"] < ohlc["range_2"]) 
    sell_conditions  = (ohlc["ema10"] < ohlc["ema40"]) & (ohlc["lag_ema10"] > ohlc["lag_ema40"]) & (ohlc["range_1"] < ohlc["range_2"])
