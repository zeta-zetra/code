...
def main(show_plot=True):
    ...
    strategy_name = "ema-rsi-fractals"
    # Read in the data
    ohlc  = read_data()
    ...
    sell_conditions = (ohlc.ema_signal==1) & (ohlc.rsi < 50) & (ohlc.william_bearish > 0) & (ohlc.Close < ohlc.ema21) & (ohlc.Close < ohlc.ema50) & (ohlc.Close < ohlc.ema200)
    buy_conditions  = (ohlc.ema_signal==2) & (ohlc.rsi > 50) & (ohlc.william_bullish > 0) & (ohlc.Close > ohlc.ema21) & (ohlc.Close > ohlc.ema50) & (ohlc.Close > ohlc.ema200)
