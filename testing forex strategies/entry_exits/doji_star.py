...
def candlestick_pivot_point_1():
    ...
    patterns  = ohlc_merged.ta.cdl_pattern(name=["dojistar"])

    ohlc_merged["dojistar"] = patterns["CDL_DOJISTAR"] 

    ohlc_merged.loc[:,"bullish_close"] = np.where(ohlc_merged["dojistar"]==100,ohlc_merged["Close"],np.nan) 
    ohlc_merged.loc[:,"bearish_close"] = np.where(ohlc_merged["dojistar"]==-100,ohlc_merged["Close"],np.nan)

    # Signal conditions
    buy_conditions  = (ohlc_merged["bullish_close"] >  ohlc_merged["R2"]) & (ohlc_merged["dojistar"]==100)
    sell_conditions = (ohlc_merged["bearish_close"] <  ohlc_merged["S2"]) & (ohlc_merged["dojistar"]==-100)
...
def candlestick_pivot_point_2():
    ...
    # Signal conditions
    buy_conditions  = (ohlc_merged["bullish_close"] <  ohlc_merged["S2"]) & (ohlc_merged["dojistar"]==100)
    sell_conditions = (ohlc_merged["bearish_close"] >  ohlc_merged["R2"]) & (ohlc_merged["dojistar"]==-100)
...
def main():
    ...
    # Calculate the patterns
    patterns  = ohlc.ta.cdl_pattern(name=["dojistar"])

    # Signal position points (i.e. for plotting)
    buy_conditions  = (patterns["CDL_DOJISTAR"]==100)
    sell_conditions = (patterns["CDL_DOJISTAR"]==-100)
