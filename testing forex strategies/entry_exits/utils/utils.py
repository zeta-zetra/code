...
def read_data():
    ...
    # Set the date 
    ohlc["date"] = pd.to_datetime(ohlc["Local time"])
...
def plot_ohlc(ohlc_subset: pd.DataFrame, filename: str, save_plot: bool = True, show_plot: bool = True ):
    ...
    if save_plot:
        dir_     = os.path.realpath('').split("entry_exits")[0]
        fig.write_image(os.path.join(dir_,"images", f"{filename}.png"))
...
def run_backtest(ohlc_subset: pd.DataFrame, strategy: Strategy, strategy_name: str = "" ) -> None:
    ...
    limited_stats.to_csv(os.path.join(dir_,"results", f"result-limited-{strategy_name}.csv"), index=False)
...
def save_backtest_stats(df: pd.DataFrame, filename: str = "") -> None:
    ...
    try:
        dir_book     = os.path.realpath('').split("code")[0]
        file_path    = os.path.join(dir_book,"book", "100-strategies", "_static","results", f"{filename}.png")
        fig.write_image(file_path)
    except Exception as e:
        pass
