...
from utils.utils import plot_ohlc, get_trading_strategy, read_data, run_backtest, plot_ohlc_pivots
...
def main(show_plot=True):
    ...
    ohlc_daily["R1"] = ohlc_daily["Close"]  + 1.1*(ohlc_daily["High"] - ohlc_daily["Low"])/12
    ohlc_daily["S1"] = ohlc_daily["Close"] - 1.1*(ohlc_daily["High"] - ohlc_daily["Low"])/12
    
    ohlc_daily["R2"] = ohlc_daily["Close"]  + 1.1*(ohlc_daily["High"] - ohlc_daily["Low"])/6
    ohlc_daily["S2"] = ohlc_daily["Close"]  - 1.1*(ohlc_daily["High"] - ohlc_daily["Low"])/6
    ...
    # Signal conditions
    buy_conditions = (ohlc_merged["Low"] <  ohlc_merged["S2"]) & ((patterns["CDL_ENGULFING"]==100) | (patterns["CDL_HARAMI"]==100))
    sell_conditions = (ohlc_merged["High"] >  ohlc_merged["R2"]) & ((patterns["CDL_ENGULFING"]==-100) | (patterns["CDL_HARAMI"]==-100))
    ...
    plot_ohlc_pivots(ohlc_merged, filename=strategy_name)   
